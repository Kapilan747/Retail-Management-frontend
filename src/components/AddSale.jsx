  ...
  return (
    <div className="add-sale-container">
      ...
      <form className="add-sale-form" onSubmit={handleSubmit}>
        <select name="customerId" value={form.customerId} onChange={handleChange} required aria-label="Select Customer" ref={customerRef} className="add-sale-select">
          <option value="">Select Customer</option>
          {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
        </select>
        <select name="productId" value={form.productId} onChange={handleChange} required aria-label="Select Product" className="add-sale-select">
          <option value="">Select Product</option>
          {products.map(p => <option key={p.id} value={p.id}>{p.name} (Stock: {p.stock})</option>)}
        </select>
        <input name="quantity" type="number" min="1" max={products.find(p => p.id === Number(form.productId))?.stock || 1} value={form.quantity} onChange={handleChange} className="add-sale-input" required aria-label="Quantity" />
        <button className="btn add-sale-btn" type="submit" aria-label="Record Sale" disabled={loading || !form.customerId || !form.productId}>Record</button>
      </form>
    </div>
  );
}
